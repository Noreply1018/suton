...
ifeq ($(CHECK),design-tokens)
	...
	@if [[ "$$CHECK" != "source-page-nav" && "$$CHECK" != "source-reader-mobile" && "$$CHECK" != "first-empty-project" && "$$CHECK" != "legacy-copy-removed" && "$$CHECK" != "legacy-frontend-removed" && "$$CHECK" != "mobile-workspace" ]]; then \
