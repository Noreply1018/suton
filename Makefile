...
ifeq ($(CHECK),design-tokens)
	...
	@if [[ "$$CHECK" != "source-page-nav" && "$$CHECK" != "current-context" && "$$CHECK" != "focus-mode" && "$$CHECK" != "source-reader-mobile" && "$$CHECK" != "first-empty-project" && "$$CHECK" != "legacy-copy-removed" && "$$CHECK" != "legacy-frontend-removed" && "$$CHECK" != "mobile-workspace" && "$$CHECK" != "question-history-long-text" && "$$CHECK" != "no-source-actions" && "$$CHECK" != "long-lists" ]]; then \
