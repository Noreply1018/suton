...
def main() -> None:
    ...
    storage_path = upload_root / f"{suffix}-stale-source.pdf"
    ...
    page_text = "stale source before stale source hit stale source after"
    source_text = "stale source hit"
    source_start = page_text.index(source_text)
    source_end = source_start + len(source_text)

    with connect() as conn:
        project = conn.execute("INSERT INTO projects (name) VALUES (%s) RETURNING id", (f"来源失效项目 {suffix}",)).fetchone()
        document = conn.execute(
            ...
            VALUES (%s, 'stale-source.pdf', 'application/pdf', %s, 1, 1, 1, 'good', true, 'completed', 'completed', now())
            RETURNING id
            """,
        ...
        chunk = conn.execute(
            ...
            VALUES (%s, %s, 1, %s, %s, %s, %s::vector, 'dashscope', 'text-embedding-v4', 1024, 'seed-stale-source')
            RETURNING id
            """,
        ...
        question = conn.execute(
            ...
            VALUES (%s, 'stale source question', 'completed', now())
            RETURNING id
            """,
            (project["id"],),
        ).fetchone()
        match = conn.execute(
            ...
            VALUES (%s, %s, %s, 1, 0.88, 1, 'strong', 'seed stale source fixture', %s, 'stale source before ', ' stale source after')
            RETURNING id
            """,
