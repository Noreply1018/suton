...
def main() -> None:
    ...
    fixed_pdf = repo_root / "tests/fixtures/text-layer-material.pdf"
    if not fixed_pdf.exists():
        raise SystemExit(f"fixture missing: {fixed_pdf}")
    upload_root = Path(settings.upload_dir).resolve()
    upload_root.mkdir(parents=True, exist_ok=True)
    missing_path = upload_root / f"{suffix}-source-reader-missing.pdf"
    if missing_path.exists():
        missing_path.unlink()
    relative_storage_path = missing_path.relative_to(repo_root).as_posix()
    zero_vector = "[" + ",".join(["0"] * 1024) + "]"
    page_text = "missing file before missing file hit missing file after"
    source_text = "missing file hit"
    source_start = page_text.index(source_text)
    source_end = source_start + len(source_text)

    with connect() as conn:
        project = conn.execute("INSERT INTO projects (name) VALUES (%s) RETURNING id", (f"来源缺失项目 {suffix}",)).fetchone()
        document = conn.execute(
            ...
            VALUES (%s, 'source-reader-missing.pdf', 'application/pdf', %s, 1, 1, 1, 'good', true, 'completed', 'completed', now())
            RETURNING id
            """,
        ...
        chunk = conn.execute(
            ...
            VALUES (%s, %s, 1, %s, %s, %s, %s::vector, 'dashscope', 'text-embedding-v4', 1024, 'seed-source-file-missing')
            RETURNING id
            """,
        ...
        question = conn.execute(
            ...
            VALUES (%s, 'source reader missing file question', 'completed', now())
            RETURNING id
            """,
            (project["id"],),
        ).fetchone()
        match = conn.execute(
            ...
            VALUES (%s, %s, %s, 1, 0.82, 1, 'strong', 'seed source file missing fixture', %s, 'missing file before ', ' missing file after')
            RETURNING id
            """,
