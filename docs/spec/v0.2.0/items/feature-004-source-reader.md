...
| 页码导航 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实文件存储、真实 PostgreSQL、Linux、Playwright 浏览器 | 已处理 `tests/fixtures/text-layer-material.pdf` 并打开来源详情 | 执行 `make verify-e2e SCENARIO=v020-source-reader-page-nav`；执行 `make verify-visual CHECK=source-page-nav` | 当前命中页突出，上一页、下一页和回到命中页可用 | E2E 已验证命中页状态、上一页、下一页、回到命中页、页码状态和 PDF hash 更新；visual 已使用真实 Web 生成 `tmp/v0.2.0-visual-evidence/1440x900-source-page-nav.png` 并检查无横向溢出 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_source_reader.py`、本地命令输出、`tmp/v0.2.0-visual-evidence/1440x900-source-page-nav.png` | 通过 |
| 移动端来源详情 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实文件存储、真实 PostgreSQL、Linux、Playwright 浏览器、390x844 viewport | 已处理 `tests/fixtures/text-layer-material.pdf` 并检索 `tests/fixtures/question.txt` 得到来源结果 | 执行 `make verify-e2e SCENARIO=v020-source-reader-mobile`；执行 `make verify-visual CHECK=source-reader-mobile` | 点击来源后打开全屏详情层；返回后当前题目和已选来源不丢失 | E2E 已验证 390x844 viewport 点击来源后打开全屏来源详情层、展示返回题目入口、来源段落和 PDF 页；点击返回后项目、来源列表和已选来源 `aria-current` 保留。visual 已生成 `tmp/v0.2.0-visual-evidence/390x844-source-reader-mobile.png`，验证全屏层覆盖视口且无横向溢出 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_source_reader.py`、本地命令输出、`tmp/v0.2.0-visual-evidence/390x844-source-reader-mobile.png` | 通过 |
