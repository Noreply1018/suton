...
- 占位阻塞契约：
  - 当前仓库尚未实现的 `make verify-visual`、除 `v020-project-document-api` 以外的 `make verify-api-contract CHECK=v020-*`、除 `v020-schema` 以外的 `make verify-db CHECK=v020-*`、`make verify-e2e SCENARIO=v020-*` 不得被空 target、只打印文本、只检查文件存在、跳过真实服务或 dry-run 替代。
