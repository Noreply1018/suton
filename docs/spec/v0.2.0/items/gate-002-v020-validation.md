...
|---|---|---|---|---|---|---|---|
| 功能总回归 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL/pgvector、真实 Redis/RQ、真实文件系统、DashScope `DASHSCOPE_API_KEY`、Linux、Playwright 浏览器 | 已执行 `make reset-demo` 和 `make migrate`；固定 fixture 存在 | 执行 `make verify-e2e SCENARIO=v020-full-regression` | v0.2.0 核心工作流全部通过 | 已落地真实 Playwright 总回归入口，串行覆盖真实项目创建、固定 PDF 上传、Redis/RQ 处理、题目检索、来源结果、来源阅读、PDF iframe、专注模式、无可靠来源行动入口和来源页码导航；该命令必须走完整 `dev_check`，当前仍需真实 DashScope 凭据和 worker 成功路径取得通过证据 | `Makefile`、`frontend/e2e/v010.spec.ts`、本地命令待补充 | 阻塞 |
| 自动化测试 | Node.js、pnpm、Python、uv、Linux、本地开发环境 | 依赖安装完成，v0.2.0 的 `SCENARIO=v020-*`、`CHECK=v020-*` 和 `make verify-visual CHECK=*` 已实现为真实验证 | 执行 `make test`；执行 `make verify-spec`；执行 `make verify-secrets` | 测试和门禁均通过，且 `make verify-spec` 覆盖 v0.2.0 结构检查 | 当前 `make test` 已通过后端 pytest、前端 typecheck、v0.2.0 DB 聚合和 API 契约聚合；`make verify-spec` 与 `make verify-secrets` 已通过；`v020-full-regression` 和 `v020-core-loop` 已落地真实入口但尚未取得真实 DashScope 成功路径通过证据，`v020-processing-progress`、`v020-document-reprocess`、`v020-document-scope-search`、`v020-question-search` 和 `v020-question-no-source` 仍未实现或未验证，因此本发布门禁行仍阻塞 | `Makefile`、`frontend/e2e/v010.spec.ts`、`scripts/verify_db.py`、`scripts/verify_api_contract.py`、`scripts/verify_release_gate.py`、`scripts/scan_secrets.py`、本地命令输出 | 阻塞 |
