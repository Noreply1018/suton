...
| 重处理与重新检索一致性 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、真实 Redis/RQ、Linux | 已处理资料并保存题目来源结果 | 执行 `make verify-db CHECK=v020-reprocess-research-consistency`；执行 `make verify-db CHECK=v020-question-research-consistency` | 资料重处理删除旧索引和旧来源；题目重新检索替换当前结果；旧题目记录保留 | 题目重新检索固定向量替换一致性已验证，资料重处理一致性未验证 | `backend/app/processing.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
