...
|---|---|---|---|---|---|---|---|
| 数据迁移 | Python、uv、真实 PostgreSQL、Linux、v0.1.0 样例数据 | 已准备 v0.1.0 数据库状态 | 执行 `make migrate`；执行 `make verify-db CHECK=v020-schema`；执行 `make verify-db CHECK=v020-project-name-migration` | 数据迁移成功，关键记录可读取 | `make migrate` 与 `make verify-db CHECK=v020-schema` 已验证当前 schema；`make verify-db CHECK=v020-project-name-migration` 已使用真实 PostgreSQL 临时数据库创建 v0.1.0 形态 `projects` 表和固定样例数据，执行当前迁移脚本后验证项目名归一化、workspace/updated_at 补齐、项目名约束、workspace/name 唯一索引和迁移后关键表可读取 | `backend/app/schema.sql`、`scripts/migrate.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 删除一致性 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、真实文件系统、Linux | 已存在项目、资料、chunk、题目和结果 | 执行 `make verify-api-contract CHECK=v020-delete-api`；执行 `make verify-db CHECK=v020-delete-consistency`；执行 `make verify-db CHECK=v020-project-hard-delete`；执行 `make verify-db CHECK=v020-document-hard-delete` | 删除后不存在可见失效来源或重复索引 | API 删除响应、固定错误文案、文件删除和失败回滚已验证；DB 删除一致性、项目硬删除和资料硬删除检查已验证；E2E 已验证删除资料后列表移除、题目记录保留且题目详情不再返回该资料来源 | `backend/app/main.py`、`frontend/e2e/v010.spec.ts`、`scripts/seed_project_counts.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 来源详情接口 | Python、uv、真实 FastAPI、真实 PostgreSQL、真实文件存储、Linux | 已处理 `tests/fixtures/text-layer-material.pdf` 并检索 `tests/fixtures/question.txt` | 执行 `make verify-db CHECK=v020-source-detail-fields` | 返回文件 ID、文件名、页码、chunk ID、片段、上下文、PDF 页入口、pgvector 相似度分数、排序位置和确定性命中原因 | API/DB 字段检查已验证；来源阅读打开 E2E 已验证前端展示这些字段和 PDF 页入口 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_source_reader.py`、`backend/app/main.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| PDF 文件接口 | Python、uv、真实 FastAPI、真实 PostgreSQL、真实文件存储、Linux | 已准备固定资料 PDF `tests/fixtures/text-layer-material.pdf` 和文件缺失资料 | 执行 `make verify-api-contract CHECK=v020-pdf-file-api` | `GET /documents/{document_id}/file` 返回 PDF 文件；资料不存在和文件缺失返回固定错误文案 | API 契约已验证固定 PDF 成功返回、资料不存在和文件缺失错误；前端 E2E 已验证 PDF 文件缺失时展示固定错误状态且不展示伪内容 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_source_file_missing.py`、`backend/app/main.py`、`scripts/verify_api_contract.py`、本地命令输出 | 通过 |
| 资料健康度与置信层级字段 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已处理 `tests/fixtures/text-layer-material.pdf`，并已生成来源结果 | 执行 `make verify-db CHECK=v020-document-health-fields`；执行 `make verify-db CHECK=v020-confidence-level-fields` | 资料接口返回健康度字段，来源结果接口返回合法 `confidence_level` | 资料健康度字段和置信层级接口字段 DB/API 检查已验证 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 失败详情字段 | Python、uv、真实 FastAPI、真实 PostgreSQL、Linux | 已上传并处理失败 `tests/fixtures/broken.pdf`；已准备固定资料 PDF `tests/fixtures/text-layer-material.pdf` | 执行 `make verify-db CHECK=v020-processing-failure-fields`；执行 `make verify-db CHECK=v020-processing-embedding-failure-stage` | 资料记录和失败详情接口返回 `failed_stage`、`failure_reason`、`failure_code` | DB/API 字段检查已验证；固定 PDF 在 embedding 凭据缺失时失败阶段记录为 `embedding` 已验证 | `backend/app/processing.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 接口契约 | Python、uv、真实 FastAPI、真实 PostgreSQL、本地文件系统、Linux | 已执行 `make migrate` 并准备项目、资料和题目样例 | 执行 `make verify-api-contract CHECK=v020-model-api` | 所有 v0.2.0 项目、资料、题目和来源接口字段、HTTP 状态码、固定错误文案符合 spec | v0.2.0 API 聚合契约已串行验证项目、资料、题目、来源详情、PDF 文件、删除、重处理、范围错误、失效来源和固定错误文案；真实 DashScope 成功路径、E2E、前端展示和视觉仍由对应功能矩阵行单独验收 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 重处理与重新检索一致性 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、真实 Redis/RQ、Linux | 已处理资料并保存题目来源结果 | 执行 `make verify-db CHECK=v020-reprocess-research-consistency`；执行 `make verify-api-contract CHECK=v020-document-reprocess-api`；执行 `make verify-db CHECK=v020-document-reprocess-no-duplicates`；执行 `make verify-db CHECK=v020-question-research-consistency` | 资料重处理删除旧索引和旧来源；题目重新检索替换当前结果；旧题目记录保留 | 资料重处理 API/DB 重置、题目重新检索固定向量替换一致性和聚合检查已验证；已落地 `v020-document-reprocess` 真实 Playwright 入口覆盖重处理后 Redis/RQ worker 再次处理完成，已落地 `v020-question-history-research` 覆盖重新检索失败路径清空旧来源，真实 DashScope 成功路径通过证据仍未取得 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、`frontend/e2e/v010.spec.ts`、本地命令输出 | 阻塞 |
| 删除暂存清理 | Python、uv、真实 FastAPI、真实 PostgreSQL、本地文件系统、Linux | 已执行项目删除和资料删除场景 | 执行 `make verify-db CHECK=v020-delete-trash-cleanup` | 已提交删除不残留可恢复来源；删除暂存目录为空或只包含可追踪失败日志记录 | DB/文件系统暂存清理检查已验证；E2E 删除资料工作流已验证 | `backend/app/main.py`、`frontend/e2e/v010.spec.ts`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 题目接口字段 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已准备题目检索、重新检索、无可靠来源和失败样例 | 执行 `make verify-api-contract CHECK=v020-question-api`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-detail-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-api-contract CHECK=v020-question-embedding-failure-api`；执行 `make verify-db CHECK=v020-question-research-consistency` | 题目详情、历史列表、重新检索响应、失败字段、置信文案和无可靠来源状态符合 spec | 题目 API 聚合契约已使用真实 FastAPI、PostgreSQL/pgvector 和固定查询向量验证新题成功、无可靠来源、重新检索成功、历史列表、详情字段、错误契约、向量失败响应和固定向量替换一致性；已落地 `v020-question-search`、`v020-question-no-source` 和 `v020-question-history-research` 真实 Playwright 入口覆盖前端新题检索、无可靠来源、历史回看和重新检索状态，真实 DashScope 成功路径通过证据仍未取得 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
