...
| 题目接口字段 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已准备题目检索、重新检索、无可靠来源和失败样例 | 执行 `make verify-api-contract CHECK=v020-question-api`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-detail-api` | 题目详情、历史列表、重新检索响应、失败字段、置信文案和无可靠来源状态符合 spec | 历史列表和题目详情字段、排序、失败字段、无可靠来源空结果和置信文案已验证，完整题目接口契约未验证 | `backend/app/main.py`、`frontend/app/page.tsx`、`scripts/verify_api_contract.py`、本地命令输出 | 阻塞 |
