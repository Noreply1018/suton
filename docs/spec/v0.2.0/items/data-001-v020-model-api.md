...
| 失败详情字段 | Python、uv、真实 FastAPI、真实 PostgreSQL、Linux | 已上传并处理失败 `tests/fixtures/broken.pdf` | 执行 `make verify-db CHECK=v020-processing-failure-fields` | 资料记录和失败详情接口返回 `failed_stage`、`failure_reason`、`failure_code` | DB/API 字段检查已验证，前端失败详情展示未验证 | `backend/app/processing.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
