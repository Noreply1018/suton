...
| 重处理与重新检索一致性 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、真实 Redis/RQ、Linux | 已处理资料并保存题目来源结果 | 执行 `make verify-db CHECK=v020-reprocess-research-consistency`；执行 `make verify-api-contract CHECK=v020-document-reprocess-api`；执行 `make verify-db CHECK=v020-document-reprocess-no-duplicates`；执行 `make verify-db CHECK=v020-question-research-consistency` | 资料重处理删除旧索引和旧来源；题目重新检索替换当前结果；旧题目记录保留 | 资料重处理 API/DB 重置、题目重新检索固定向量替换一致性和聚合检查已验证；`v020-document-reprocess` 已在有效 DashScope 凭据环境中通过，覆盖重处理后 Redis/RQ worker 再次处理完成；`v020-question-history-research` 已覆盖重新检索失败路径清空旧来源 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、`frontend/e2e/v010.spec.ts`、本地命令输出 | 通过 |
