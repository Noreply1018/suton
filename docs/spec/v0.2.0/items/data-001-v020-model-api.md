...
| 题目接口字段 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已准备题目检索、重新检索、无可靠来源和失败样例 | 执行 `make verify-api-contract CHECK=v020-question-api`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-detail-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-api-contract CHECK=v020-question-embedding-failure-api`；执行 `make verify-db CHECK=v020-question-research-consistency` | 题目详情、历史列表、重新检索响应、失败字段、置信文案和无可靠来源状态符合 spec | 历史列表、题目详情、重新检索入口、错误契约、题目向量失败响应和固定向量替换一致性已验证，完整题目接口契约未验证 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
