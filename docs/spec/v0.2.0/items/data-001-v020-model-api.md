...
- 类型：数据与接口变更
- 状态：草案
- 背景：v0.2.0 增加项目重命名/删除、资料删除/重处理、处理进度、题目历史和来源详情，需要固定数据与接口语义。
- 当前问题：如果接口语义不固定，前端重构会依赖临时行为，资料删除、重新处理和来源详情容易出现不一致。
- 目标行为：v0.2.0 固定项目、资料、处理状态、题目和来源详情的数据语义。所有会影响删除、重处理、唯一性和来源展示的行为必须可验证。
- 非目标：不包含多用户租户模型、权限模型、对象存储模型、云同步模型。
- 发布必要性：必须发布
- 用户可见影响：用户操作项目、资料和题目时状态一致，不出现删除后仍可检索、重处理重复结果或历史结果打不开的混乱体验。
- 涉及模块：数据库、后端 API、后台任务、检索、前端数据访问。
- 配置、接口或数据结构变化：必须新增或调整项目唯一性、资料处理阶段、资料删除/重处理、题目历史、来源详情相关字段和接口。
