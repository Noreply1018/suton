...
| PDF 文件接口 | Python、uv、真实 FastAPI、真实 PostgreSQL、真实文件存储、Linux | 已准备固定资料 PDF `tests/fixtures/text-layer-material.pdf` 和文件缺失资料 | 执行 `make verify-api-contract CHECK=v020-pdf-file-api` | `GET /documents/{document_id}/file` 返回 PDF 文件；资料不存在和文件缺失返回固定错误文案 | API 契约已验证固定 PDF 成功返回、资料不存在和文件缺失错误；前端 PDF 阅读未验证 | `backend/app/main.py`、`scripts/verify_api_contract.py`、本地命令输出 | 阻塞 |
