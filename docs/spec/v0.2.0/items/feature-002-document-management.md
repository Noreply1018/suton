...
| 重新处理 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 Redis/RQ、真实 PostgreSQL/pgvector、DashScope `DASHSCOPE_API_KEY`、Linux、Playwright 浏览器 | 已上传并处理 `tests/fixtures/text-layer-material.pdf` | 执行 `make verify-e2e SCENARIO=v020-document-reprocess`；执行 `make verify-db CHECK=v020-document-reprocess-no-duplicates` | 旧索引被替换，不出现重复 chunk，旧来源结果不再展示 | DB 重置检查已验证旧页面/chunk/来源清理、原 PDF 保留和题目保留，真实 Redis/RQ 完整重处理与 E2E 未验证 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
