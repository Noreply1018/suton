...
| 回归核心闭环 | Node.js、pnpm、Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、真实 Redis/RQ、DashScope `DASHSCOPE_API_KEY`、Linux、Playwright 浏览器 | 已执行 `make reset-demo` 和 `make migrate` | 执行 `make verify-e2e SCENARIO=v020-core-loop` | 新前端完成项目、资料、题目、来源结果闭环 | 已落地真实 Playwright 场景入口，覆盖项目创建、固定 PDF 上传、Redis/RQ 处理完成、题目检索、来源结果、来源阅读、PDF iframe、专注模式进入和恢复；该场景必须走完整 `dev_check`，当前仍需真实 DashScope 凭据和 worker 成功路径完成验证 | `Makefile`、`frontend/e2e/v010.spec.ts`、本地命令待补充 | 阻塞 |
