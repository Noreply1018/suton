...
| 题目接口契约 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已准备项目、可检索资料、无匹配题目和失败注入场景 | 执行 `make verify-api-contract CHECK=v020-question-api`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-detail-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-db CHECK=v020-question-research-consistency`；执行 `make verify-db CHECK=v020-reprocess-research-consistency` | 新题检索、重新检索、历史列表和题目详情响应字段、错误文案、状态和无可靠来源契约符合 spec | 历史列表、题目详情、重新检索错误子契约、固定向量替换一致性和重处理/重新检索聚合一致性已验证，新题检索成功路径和完整题目接口契约未验证 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
