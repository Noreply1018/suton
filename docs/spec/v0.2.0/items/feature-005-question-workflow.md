...
| 历史与重新检索 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器 | 已存在多道题目记录 | 执行 `make verify-e2e SCENARIO=v020-question-history-research`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-db CHECK=v020-question-research-consistency` | 历史题目展示最近一次检索结果；重新检索后替换该题当前可见结果 | 历史 API 字段、排序、项目隔离、重新检索错误契约和固定向量替换一致性已验证，E2E 未验证 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
...
| 题目接口契约 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已准备项目、可检索资料、无匹配题目和失败注入场景 | 执行 `make verify-api-contract CHECK=v020-question-api`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-detail-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-db CHECK=v020-question-research-consistency` | 新题检索、重新检索、历史列表和题目详情响应字段、错误文案、状态和无可靠来源契约符合 spec | 历史列表、题目详情、重新检索错误子契约和固定向量替换一致性已验证，新题检索成功路径和完整题目接口契约未验证 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
