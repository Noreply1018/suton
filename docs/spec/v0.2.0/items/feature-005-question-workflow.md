...
| 历史与重新检索 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器 | 已存在多道题目记录 | 执行 `make verify-e2e SCENARIO=v020-question-history-research`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-db CHECK=v020-question-research-consistency`；执行 `make verify-db CHECK=v020-reprocess-research-consistency` | 历史题目展示最近一次检索结果；重新检索后替换该题当前可见结果 | 历史 API 字段、排序、项目隔离、重新检索错误契约、固定向量替换一致性，以及资料重处理清旧来源与重新检索替换结果的串行聚合检查已验证；E2E 未验证 | `backend/app/main.py`、`backend/app/processing.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
