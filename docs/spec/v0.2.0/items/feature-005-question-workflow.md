...
|---|---|---|---|---|---|---|---|
| 新题检索 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL/pgvector、DashScope `DASHSCOPE_API_KEY`、Linux、Playwright 浏览器 | 已上传并处理 `tests/fixtures/text-layer-material.pdf` | 执行 `make verify-e2e SCENARIO=v020-question-search QUESTION=tests/fixtures/question.txt` | 请求未完成时展示 `LoaderCircle` 和 `正在检索来源`，按钮禁用；完成后返回带来源结果和置信层级 | `make verify-e2e SCENARIO=v020-question-search` 已在有效 DashScope 凭据环境中通过，覆盖真实 UI 创建项目、上传固定 PDF、等待 worker 完成索引、提交固定题目、真实请求未完成时展示 `正在检索来源` 且按钮禁用、响应为 `completed`、失败字段为空、返回来源均来自该资料且置信文案属于固定三档 | `frontend/e2e/v010.spec.ts`、本地命令输出 | 通过 |
| 无可靠来源 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL/pgvector、DashScope `DASHSCOPE_API_KEY`、Linux、Playwright 浏览器 | 已准备 `tests/fixtures/unmatched-question.txt` | 执行 `make verify-e2e SCENARIO=v020-question-no-source QUESTION=tests/fixtures/unmatched-question.txt` | 页面展示无可靠来源状态和三个行动入口，不展示猜测答案 | `make verify-e2e SCENARIO=v020-question-no-source` 已在有效 DashScope 凭据环境中通过，覆盖真实 UI 创建项目、上传固定 PDF、等待 worker 完成索引、提交固定无匹配题目、响应为 `no_reliable_source`、失败字段为空、`matches` 为空、页面展示固定无可靠来源文案和三个行动入口，且不展示来源卡或 pgvector 相似度 | `frontend/e2e/v010.spec.ts`、本地命令输出 | 通过 |
| 历史与重新检索 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器 | 已存在多道题目记录 | 执行 `make verify-e2e SCENARIO=v020-question-history-research`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-db CHECK=v020-question-research-consistency`；执行 `make verify-db CHECK=v020-reprocess-research-consistency` | 历史题目展示最近一次检索结果；重新检索后替换该题当前可见结果 | 已使用真实 Web、真实 FastAPI 和真实 PostgreSQL 验证历史题目回看最近一次来源结果；前端点击重新检索后调用真实 `POST /questions/{question_id}/research`，在缺少 embedding 凭据的真实失败路径中清空旧来源并展示后端 `failure_reason`；历史 API 字段、排序、项目隔离、重新检索错误契约、固定向量成功替换一致性，以及资料重处理清旧来源与重新检索替换结果的串行聚合检查已验证 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_question_history_research.py`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、`Makefile`、本地命令输出 | 通过 |
| 置信层级 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL/pgvector、Linux、Playwright 浏览器 | 验证脚本在真实 PostgreSQL/pgvector 中创建 1 个测试项目、1 份测试资料、3 个固定 1024 维向量 chunk 和 1 个固定查询向量，三条候选 score 分别落入 `strong`、`reference`、`low` 区间；该测试不调用 embedding provider | 执行 `make verify-e2e SCENARIO=v020-confidence-levels`；执行 `make verify-db CHECK=v020-confidence-levels`；执行 `make verify-db CHECK=v020-confidence-level-fields` | 来源结果按固定向量 score 生成 `strong`、`reference`、`low`，前端展示强相关、可参考、低置信 | DB 阈值、接口字段和前端三档 `StatusPill` 展示已验证；该场景不调用 embedding provider | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_confidence_levels.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 检索范围错误 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器 | 已存在当前项目和至少一份不可用或跨项目资料 | 执行 `make verify-e2e SCENARIO=v020-question-scope-errors`；执行 `make verify-api-contract CHECK=v020-question-scope-errors` | 空范围、跨项目资料、未完成资料和不可检索资料均返回固定错误，页面不产生伪结果 | 已使用真实 Web、真实 FastAPI 和真实 PostgreSQL 验证前端提交题目时，空范围、跨项目资料、处理中资料和不可检索资料均展示固定错误文案，不产生来源卡片或题目历史；API 契约同步覆盖固定错误响应 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/e2e/v010.spec.ts`、`scripts/seed_question_scope_errors.py`、`scripts/verify_api_contract.py`、`Makefile`、本地命令输出 | 通过 |
| 长题目历史布局 | Node.js、pnpm、真实 Web、Linux、Playwright 浏览器、390x844 和 1440x900 viewport | 已创建包含长题干的多道历史题目 | 执行 `make verify-e2e SCENARIO=v020-question-history-long-text`；执行 `make verify-visual CHECK=question-history-long-text` | 历史列表不撑开页面，不遮挡操作入口，长文本截断或换行符合布局 | 已使用真实 Web、真实 FastAPI、真实 PostgreSQL 和 20 条长题干历史验证桌面/移动端内部滚动、无横向溢出、单条历史高度受控、选择历史题目后展示无来源状态，并生成 1440x900 与 390x844 截图证据 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_question_history_long_text.py`、`Makefile`、本地命令输出、`tmp/v0.2.0-visual-evidence/1440x900-question-history-long-text.png`、`tmp/v0.2.0-visual-evidence/390x844-question-history-long-text.png` | 通过 |
| 题目接口契约 | Python、uv、真实 FastAPI、真实 PostgreSQL/pgvector、Linux | 已准备项目、可检索资料、无匹配题目和失败注入场景 | 执行 `make verify-api-contract CHECK=v020-question-api`；执行 `make verify-api-contract CHECK=v020-question-history-api`；执行 `make verify-api-contract CHECK=v020-question-detail-api`；执行 `make verify-api-contract CHECK=v020-question-research-scope-errors`；执行 `make verify-api-contract CHECK=v020-question-embedding-failure-api`；执行 `make verify-db CHECK=v020-question-research-consistency`；执行 `make verify-db CHECK=v020-reprocess-research-consistency` | 新题检索、重新检索、历史列表和题目详情响应字段、错误文案、状态和无可靠来源契约符合 spec | 题目 API 聚合契约已使用真实 FastAPI、PostgreSQL/pgvector 和固定查询向量验证新题成功、无可靠来源、重新检索成功、历史列表、详情字段、错误契约、向量失败响应和旧来源替换；新题检索、无可靠来源、历史回看和重新检索前端真实 Playwright 入口已落地，其中 `v020-question-search` 和 `v020-question-no-source` 已在有效 DashScope 凭据环境中通过 | `backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/verify_api_contract.py`、`scripts/verify_db.py`、本地命令输出 | 通过 |
| 无可靠来源行动入口 | Node.js、pnpm、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器、390x844 和 1440x900 viewport | 已检索 `tests/fixtures/unmatched-question.txt` 得到无可靠来源状态 | 执行 `make verify-e2e SCENARIO=v020-no-source-actions`；执行 `make verify-visual CHECK=no-source-actions` | 三个行动入口分别打开范围选择、跳转资料索引并高亮不可检索资料、聚焦题目输入框并保留原题文本 | 已使用真实 Web、真实 FastAPI 和真实 PostgreSQL seed 验证无可靠来源状态不生成答案，并验证三个行动入口分别打开指定资料范围、定位并高亮不可检索资料、保留原题文本并聚焦输入框；已生成 1440x900 与 390x844 截图证据 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_no_source_actions.py`、`Makefile`、本地命令输出、`tmp/v0.2.0-visual-evidence/1440x900-no-source-actions.png`、`tmp/v0.2.0-visual-evidence/390x844-no-source-actions.png` | 通过 |
