...
|---|---|---|---|---|---|---|---|
| 正常处理进度 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 Redis/RQ、真实 PostgreSQL、DashScope `DASHSCOPE_API_KEY`、Linux、Playwright 浏览器 | 已创建项目并准备 `tests/fixtures/text-layer-material.pdf` | 执行 `make verify-e2e SCENARIO=v020-processing-progress`；执行 `make verify-db CHECK=v020-processing-stages`；执行 `make verify-visual CHECK=processing-progress-visual` | 页面展示固定“纸页入库”组件、真实上传进度线、真实处理轨道，并最终完成 | 已新增并通过 `make verify-db CHECK=v020-processing-stages`：使用真实 `process_document`、真实 PDF 文字层提取和真实 PostgreSQL/pgvector 写入，在 embedding 与 indexing 中间阶段直接查库验证 `processing_stage` 先写入后推进，并验证最终 `completed` 状态、chunk 持久化和失败字段清空；已新增并通过 `make verify-visual CHECK=processing-progress-visual`：使用真实 Web、FastAPI、PostgreSQL seed 生成处理轨道运行中截图并检查无视觉硬错误；已落地 `make verify-e2e SCENARIO=v020-processing-progress` 真实 Playwright 入口，使用真实 Web、FastAPI、Redis/RQ、PostgreSQL 和固定 PDF 上传，检查纸页入库组件、处理轨道六节点、最终 `completed`、失败字段清空、可检索和 chunk 持久化；真实 DashScope 成功路径通过证据仍未取得 | `backend/app/processing.py`、`scripts/verify_db.py`、`frontend/e2e/v010.spec.ts`、`Makefile`、`tmp/v0.2.0-visual-evidence/1440x900-processing-progress-visual.png`、本地命令输出 | 阻塞 |
