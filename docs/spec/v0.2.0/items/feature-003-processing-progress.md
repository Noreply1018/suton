...
| 失败状态 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器 | 准备 `tests/fixtures/broken.pdf` 和固定资料 PDF `tests/fixtures/text-layer-material.pdf` | 执行 `make verify-e2e SCENARIO=v020-processing-failure`；执行 `make verify-db CHECK=v020-processing-failure-fields`；执行 `make verify-db CHECK=v020-processing-embedding-failure-stage` | 页面展示失败节点、后端 `failure_reason` 原文、重新处理、删除资料、查看失败原因入口 | DB/API 失败字段检查已验证；固定 PDF 在 embedding 凭据缺失时失败阶段记录为 `embedding` 已验证；E2E/前端失败状态未验证 | `backend/app/processing.py`、`scripts/verify_db.py`、本地命令输出 | 阻塞 |
