...
| 失败状态 | Node.js、pnpm、Python、uv、真实 Web、真实 FastAPI、真实 PostgreSQL、Linux、Playwright 浏览器 | 准备 `tests/fixtures/broken.pdf` | 执行 `make verify-e2e SCENARIO=v020-processing-failure` | 页面展示失败节点、后端 `failure_reason` 原文、重新处理、删除资料、查看失败原因入口 | 未验证 | 待补充 | 阻塞 |
