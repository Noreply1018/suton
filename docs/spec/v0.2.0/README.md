...
当前可本机推进的 API、DB、视觉、前端非 embedding 场景和证据包生成器切片已落地到真实 target；剩余阻塞不得降级为 mock、固定向量成功路径或 `--skip-embedding`。最后集中向用户确认时，必须先取得并在运行环境中设置有效 DashScope `DASHSCOPE_API_KEY`，再重新执行以下真实成功路径命令并归档证据：
...
这些命令通过前，相关功能矩阵、证据归档、发布前审计和 v0.2.0 整体发布门禁必须保持阻塞。任何证据包、日志、提交、文档和最终回复都不得包含真实 API key、token、secret 或 password。
...
| 2026-06-02 | 同步总回归与题目工作流阻塞证据：根据最新 `make evidence-package-with-tests` 生成的 `tmp/v0.2.0-evidence-latest.md`，明确总回归入口当前因缺少 `DASHSCOPE_API_KEY` 失败于 `dev_check`；同步 `feature-005` 题目接口契约行的前端入口状态，结论继续保持阻塞。 | v0.2.0 spec 状态维护 | `items/design-001-frontend-rebuild.md`、`items/feature-005-question-workflow.md`、`items/gate-002-v020-validation.md`、`tmp/v0.2.0-evidence-latest.md` |
