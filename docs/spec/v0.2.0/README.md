...
| 条目 | 状态 | 当前结论 | 下一步 |
|---|---|---|---|
| `design-001-frontend-rebuild.md` | 草案 | 视觉方向和重构边界已固定，组件级实现契约仍需继续细化 | 固定设计 token、核心组件清单和旧前端删除判据 |
| `design-002-workspace-layout.md` | 草案 | 桌面、移动和专注模式布局约束已固定 | 补充布局尺寸、滚动容器和响应式断点细则 |
| `feature-001-project-management.md` | 草案 | 项目命名、唯一性、重命名和硬删除语义已固定 | 补充接口错误码和删除失败事务判据 |
| `feature-002-document-management.md` | 草案 | 资料删除、重处理、健康度和检索范围语义已固定 | 补充资料详情响应字段和重处理任务状态流转 |
| `feature-003-processing-progress.md` | 草案 | 上传进度和后台处理轨道语义已固定 | 补充阶段枚举、失败码和前端状态恢复规则 |
| `feature-004-source-reader.md` | 草案 | 来源详情、PDF 页码导航和失败状态已固定 | 与数据接口条目保持字段级一致 |
| `feature-005-question-workflow.md` | 草案 | 历史题目、重新检索、无可靠来源和置信层级已固定 | 补充请求/响应契约和历史结果替换规则 |
| `data-001-v020-model-api.md` | 草案 | 数据模型、接口形状和迁移规则正在冻结 | 继续消除字段、错误码和删除事务歧义 |
| `gate-001-visual-quality.md` | 草案 | 视觉截图和人工审美门禁已固定 | 补充截图归档路径和审计记录模板 |
| `gate-002-v020-validation.md` | 草案 | 总体验收命令和发布阻塞原则已固定 | 补齐 v020 验证命令真实实现前的占位检查 |
...
- 桌面宽度截图：1440px、1280px。
- 平板或窄屏截图：1024px。
- 移动宽度截图：390px。
- 主要状态截图：首次空项目、新建项目、资料列表、资料健康度、纸页入库上传中、处理轨道运行中、处理失败修复入口、题目检索、来源置信层级、无可靠来源、专注模式、来源详情、PDF 页码导航、PDF 阅读视图。
...
- 涉及远端状态、tag、Release 或镜像时，必须通过可追溯外部证据核验。
