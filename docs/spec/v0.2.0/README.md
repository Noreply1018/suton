...
近期新增已实现 `make verify-e2e SCENARIO=v020-upload-indeterminate-progress`、`make verify-visual CHECK=upload-indeterminate-progress`、`make verify-e2e SCENARIO=v020-question-scope-errors` 和 `make verify-e2e SCENARIO=v020-question-history-research`，分别覆盖浏览器上传字节不可计算时的非百分比流动线、题目检索范围错误，以及历史题目回看和重新检索前端 E2E；其余未实现命令仍按上段约束保持阻塞。
