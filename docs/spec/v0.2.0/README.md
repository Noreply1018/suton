...
当前 `make verify-spec` 对 v0.2.0 执行草案结构检查、已实现 target inventory 检查和集中阻塞清单检查，用于防止缺文件、缺字段、核心约束缺失、README 已实现清单与源码支持 target 漂移、真实 DashScope 成功路径场景误入 `--skip-embedding` 白名单，以及剩余 DashScope 阻塞命令或有效凭据/no-mock/no-secret 约束从集中清单漂移；它不代表 v0.2.0 功能、视觉、数据库或 E2E 验证已经实现。v0.2.0 实现期必须把本 spec 中列出的 `SCENARIO=v020-*`、`CHECK=v020-*` 和 `make verify-visual CHECK=*` 全部落成真实验证，再将条目状态更新为已完成。

当前仓库已实现 `make verify-db CHECK=v020-schema`、`make verify-db CHECK=v020-confidence-levels`、`make verify-db CHECK=v020-confidence-level-fields`、`make verify-db CHECK=v020-project-name-migration`、`make verify-db CHECK=v020-project-hard-delete`、`make verify-db CHECK=v020-document-hard-delete`、`make verify-db CHECK=v020-delete-consistency`、`make verify-db CHECK=v020-delete-trash-cleanup`、`make verify-db CHECK=v020-document-reprocess-no-duplicates`、`make verify-db CHECK=v020-document-health-fields`、`make verify-db CHECK=v020-document-health`、`make verify-db CHECK=v020-processing-failure-fields`、`make verify-db CHECK=v020-processing-embedding-failure-stage`、`make verify-db CHECK=v020-processing-stages`、`make verify-db CHECK=v020-source-detail-fields`、`make verify-db CHECK=v020-question-research-consistency`、`make verify-db CHECK=v020-reprocess-research-consistency`、`make verify-api-contract CHECK=v020-project-document-api`、`make verify-api-contract CHECK=v020-document-detail-fields`、`make verify-api-contract CHECK=v020-document-reprocess-api`、`make verify-api-contract CHECK=v020-delete-api`、`make verify-api-contract CHECK=v020-document-scope-disabled`、`make verify-api-contract CHECK=v020-pdf-file-api`、`make verify-api-contract CHECK=v020-project-name-limits`、`make verify-api-contract CHECK=v020-question-scope-errors`、`make verify-api-contract CHECK=v020-question-history-api`、`make verify-api-contract CHECK=v020-question-detail-api`、`make verify-api-contract CHECK=v020-question-research-scope-errors`、`make verify-api-contract CHECK=v020-question-embedding-failure-api`、`make verify-api-contract CHECK=v020-question-api`、`make verify-api-contract CHECK=v020-stale-source`、`make verify-api-contract CHECK=v020-model-api`、`make verify-e2e SCENARIO=v020-first-empty-project`、`make verify-e2e SCENARIO=v020-project-create`、`make verify-e2e SCENARIO=v020-project-unique-name`、`make verify-e2e SCENARIO=v020-project-name-limits`、`make verify-e2e SCENARIO=v020-project-rename-delete`、`make verify-e2e SCENARIO=v020-project-delete-selection`、`make verify-e2e SCENARIO=v020-document-delete`、`make verify-e2e SCENARIO=v020-document-health`、`make verify-e2e SCENARIO=v020-document-detail-fields`、`make verify-e2e SCENARIO=v020-document-scope-disabled`、`make verify-e2e SCENARIO=v020-document-scope-search`、`make verify-e2e SCENARIO=v020-document-reprocess`、`make verify-e2e SCENARIO=v020-processing-failure`、`make verify-e2e SCENARIO=v020-processing-refresh`、`make verify-e2e SCENARIO=v020-processing-polling-coalesced`、`make verify-e2e SCENARIO=v020-processing-progress`、`make verify-e2e SCENARIO=v020-source-reader-open`、`make verify-e2e SCENARIO=v020-source-reader-switch`、`make verify-e2e SCENARIO=v020-source-reader-page-nav`、`make verify-e2e SCENARIO=v020-source-reader-mobile`、`make verify-e2e SCENARIO=v020-source-reader-file-missing`、`make verify-e2e SCENARIO=v020-source-reader-stale-source`、`make verify-e2e SCENARIO=v020-confidence-levels`、`make verify-e2e SCENARIO=v020-question-history-long-text`、`make verify-e2e SCENARIO=v020-question-search`、`make verify-e2e SCENARIO=v020-question-no-source`、`make verify-e2e SCENARIO=v020-no-source-actions`、`make verify-e2e SCENARIO=v020-long-lists`、`make verify-e2e SCENARIO=v020-focus-mode`、`make verify-e2e SCENARIO=v020-focus-mode-restore`、`make verify-e2e SCENARIO=v020-core-loop`、`make verify-e2e SCENARIO=v020-full-regression`、`make verify-visual CHECK=first-empty-project`、`make verify-visual CHECK=legacy-copy-removed`、`make verify-visual CHECK=legacy-frontend-removed`、`make verify-visual CHECK=mobile-workspace`、`make verify-visual CHECK=source-page-nav`、`make verify-visual CHECK=current-context`、`make verify-visual CHECK=focus-mode`、`make verify-visual CHECK=workspace-breakpoints`、`make verify-visual CHECK=visual-system`、`make verify-visual CHECK=screenshot-matrix`、`make verify-visual CHECK=visual-hard-errors`、`make verify-visual CHECK=source-reader-mobile`、`make verify-visual CHECK=question-history-long-text`、`make verify-visual CHECK=no-source-actions`、`make verify-visual CHECK=long-lists`、`make verify-visual CHECK=processing-progress-visual` 和 `make verify-visual CHECK=design-tokens`，但尚未实现其余 `make verify-visual CHECK=*`、`CHECK=v020-*` / `SCENARIO=v020-*` 检查；其中 `v020-document-reprocess`、`v020-document-scope-search`、`v020-question-search`、`v020-question-no-source`、`v020-processing-progress`、`v020-core-loop` 和 `v020-full-regression` 已在有效 DashScope 凭据环境中取得真实成功路径通过证据。因此引用未实现命令或未完整覆盖场景的 v0.2.0 验证矩阵行必须保持 `未验证 / 待补充 / 阻塞`。进入 v0.2.0 实现验收前，必须先把这些命令作为真实 Make target 或真实 `CHECK` 分支落地，且不得用空脚本、mock、dry-run 或只检查文件存在的脚本替代真实验证。

近期新增已实现 `make verify-e2e SCENARIO=v020-upload-indeterminate-progress`、`make verify-visual CHECK=upload-indeterminate-progress`、`make verify-e2e SCENARIO=v020-question-scope-errors`、`make verify-e2e SCENARIO=v020-question-history-research`、`make verify-visual CHECK=visual-evidence-manifest`、`make verify-visual CHECK=aesthetic-audit-record`、`make verify-visual CHECK=visual-hard-errors`、`make verify-visual CHECK=screenshot-matrix`、`make verify-visual CHECK=visual-system`、强化后的 `make verify-visual CHECK=design-tokens`、`make verify-db CHECK=v020-processing-stages`、`make verify-visual CHECK=processing-progress-visual`、`make verify-e2e SCENARIO=v020-processing-progress`、`make verify-e2e SCENARIO=v020-document-reprocess`、`make verify-e2e SCENARIO=v020-document-scope-search`、`make verify-e2e SCENARIO=v020-question-search`、`make verify-e2e SCENARIO=v020-question-no-source`、`make verify-e2e SCENARIO=v020-core-loop` 和 `make verify-e2e SCENARIO=v020-full-regression`；其中前四项覆盖浏览器上传字节不可计算时的非百分比流动线、题目检索范围错误、历史题目回看和重新检索前端 E2E，`screenshot-matrix` 使用真实浏览器生成 16 张截图并覆盖 16 个固定状态和 5 个固定 viewport，`visual-hard-errors` 使用真实浏览器覆盖五个固定 viewport 的布局硬错误，manifest 和审美审计记录为视觉发布门禁的真实文件校验 target，`visual-system` 串行聚合截图矩阵、硬错误、manifest 和审美审计，`design-tokens` 现在同时校验固定 token、lucide 图标规则、禁用主 UI 组件库和统一组件清单定义/使用，`v020-processing-stages` 使用真实 `process_document` 和 PostgreSQL/pgvector 验证处理阶段推进，`processing-progress-visual` 使用真实 Web/API/DB seed 生成处理轨道运行中截图，`v020-processing-progress` 已在有效 DashScope 凭据环境中通过并覆盖固定 PDF 上传、纸页入库、Redis/RQ worker、处理轨道六节点和最终完成状态，`v020-document-reprocess` 已在有效 DashScope 凭据环境中通过并覆盖已完成资料重处理、重新处理中标记和 worker 再次完成，`v020-document-scope-search` 已在有效 DashScope 凭据环境中通过并覆盖两份固定 PDF 上传、指定资料多选、请求体范围参数和响应来源资料约束，`v020-question-search` 已在有效 DashScope 凭据环境中通过并覆盖固定 PDF 上传处理、题目检索 loading、按钮禁用、来源结果和置信层级，`v020-question-no-source` 已在有效 DashScope 凭据环境中通过并覆盖固定无匹配题目、no_reliable_source 响应、无可靠来源行动入口和不展示伪来源，`v020-core-loop` 和 `v020-full-regression` 已在有效 DashScope 凭据环境中通过并覆盖上传处理、题目检索、来源阅读、专注模式、无可靠来源和页码导航；其中截图矩阵、manifest、视觉硬错误、人工审美审计记录、视觉系统聚合门禁、设计 token/组件清单门禁、处理阶段 DB 门禁、处理进度视觉门禁、上述真实 DashScope E2E 成功路径、证据包归档和本轮完整自动化复跑已通过，发布前审计和提交仍保持阻塞；其余未实现命令仍按上段约束保持阻塞。

视觉发布门禁中的 `make verify-visual CHECK=screenshot-matrix`、`make verify-visual CHECK=visual-hard-errors`、`make verify-visual CHECK=visual-evidence-manifest` 和 `make verify-visual CHECK=aesthetic-audit-record` 已落地并通过真实检查；这只代表视觉质量门禁的当前截图矩阵、硬错误、manifest 和人工审美审计切片通过，不代表发布前审计或 v0.2.0 整体发布门禁完成。
...
当前 `make evidence-package-with-tests` 已切换为 v0.2.0 证据包生成器，会写入固定 `tmp/v0.2.0-evidence-latest.md` 并记录视觉证据摘要、每条必需命令的退出码、执行时间、数据准备命令、证据路径和结论。v0.2.0 视觉证据摘要必须在证据命令执行后读取，以反映本轮 `screenshot-matrix` 生成后的最新 manifest。当前真实 DashScope E2E 成功路径已取得本地通过证据，且 `make evidence-package-with-tests` 已重新生成证据包并通过；整体发布门禁仍需完成发布前 subagent 严格审计和提交。

## 集中阻塞确认清单

当前可本机推进的 API、DB、视觉、前端非 embedding 场景和证据包生成器切片已落地到真实 target；真实 DashScope E2E 成功路径已在有效 `DASHSCOPE_API_KEY` 运行环境中重新执行并通过。后续回归、证据包重跑和发布前审计不得降级为 mock、固定向量成功路径或 `--skip-embedding`。后续回归或证据包重跑时必须继续在运行环境中设置有效 DashScope `DASHSCOPE_API_KEY`，并追踪以下真实成功路径命令和证据归档命令：

- `make verify-e2e SCENARIO=v020-processing-progress`
- `make verify-e2e SCENARIO=v020-document-reprocess`
- `make verify-e2e SCENARIO=v020-document-scope-search`
- `make verify-e2e SCENARIO=v020-question-search`
- `make verify-e2e SCENARIO=v020-question-no-source`
- `make verify-e2e SCENARIO=v020-core-loop`
- `make verify-e2e SCENARIO=v020-full-regression`
- `make evidence-package-with-tests`

subagent 严格审计和提交完成前，发布前审计和 v0.2.0 整体发布门禁必须保持阻塞。任何证据包、日志、提交、文档和最终回复都不得包含真实 API key、token、secret 或 password。
...
| 时间 | 变更 | 来源 | 证据 |
|---|---|---|---|
| 2026-06-02 | 解除真实 DashScope E2E 成功路径集中阻塞：`v020-processing-progress`、`v020-document-reprocess`、`v020-document-scope-search`、`v020-question-search`、`v020-question-no-source`、`v020-core-loop` 和 `v020-full-regression` 已在有效凭据环境中通过；总回归无可靠来源断言对齐 v0.2.0 固定文案并保留不展示来源卡约束。`make test`、`make verify-spec`、`make verify-secrets` 和 `make evidence-package-with-tests` 已通过；subagent 审计和提交仍保持阻塞。 | v0.2.0 实现推进 | `frontend/e2e/v010.spec.ts`、`backend/app/main.py`、`scripts/reset_demo.py`、`items/gate-002-v020-validation.md`、`tmp/v0.2.0-evidence-latest.md` |
...
| 2026-06-02 | 同步数据模型与接口契约局部状态：根据已落地的 `v020-model-api`、删除 API/DB、资料健康度、置信层级、处理失败字段、删除暂存清理和 `v020-document-delete` E2E 证据，将 `data-001-v020-model-api.md` 中对应局部行更新为通过；当时完整 v0.1.0 样例迁移、重处理 worker 完成、题目 E2E 和真实 DashScope 成功路径保持阻塞。 | v0.2.0 spec 状态维护 | `items/data-001-v020-model-api.md` |
...
| 2026-06-02 | 落地长列表布局切片：新增 `make verify-e2e SCENARIO=v020-long-lists` 和 `make verify-visual CHECK=long-lists`，使用真实 Web、FastAPI 和 PostgreSQL seed 验证 20 个项目、20 份资料、20 道题目历史和 20 条来源结果列表均为区域内滚动，桌面/移动无横向溢出并生成截图证据；总视觉门禁仍未验证。 | v0.2.0 实现推进 | `Makefile`、`backend/app/main.py`、`backend/app/processing.py`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_long_lists.py`、`items/design-002-workspace-layout.md`、`items/feature-005-question-workflow.md` |
...
| 2026-06-02 | 落地专注模式与恢复切片：新增 `make verify-e2e SCENARIO=v020-focus-mode`、`make verify-e2e SCENARIO=v020-focus-mode-restore` 和 `make verify-visual CHECK=focus-mode`，使用真实 Web、FastAPI 和 PostgreSQL seed 验证进入后隐藏项目栏、资料管理、题目输入和题目历史，仅保留当前题目、来源结果和 PDF 阅读；退出后项目、题目、来源、PDF 页码、检索范围和资料列表滚动位置保持，并生成 `tmp/v0.2.0-visual-evidence/1440x900-focus-mode.png`；总视觉门禁仍未验证。 | v0.2.0 实现推进 | `Makefile`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_source_reader.py`、`scripts/seed_long_lists.py`、`items/design-002-workspace-layout.md` |
| 2026-06-02 | 落地桌面断点矩阵切片：新增 `make verify-visual CHECK=workspace-breakpoints`，使用真实 Web、FastAPI 和 PostgreSQL source reader seed 验证 1440x900、1280x832、1200x800 和 1024x768 下的三列、紧凑三列和双栏断点列宽，确认 1024x768 来源 fixed 覆盖层可点击打开 PDF 阅读，无浏览器级横向/纵向滚动，并生成四个 viewport 截图证据；总视觉门禁仍未验证。 | v0.2.0 实现推进 | `Makefile`、`frontend/app/globals.css`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_source_reader.py`、`items/design-002-workspace-layout.md` |
| 2026-06-02 | 落地题目检索范围错误前端 E2E 切片：新增 `make verify-e2e SCENARIO=v020-question-scope-errors`，使用真实 Web、FastAPI 和 PostgreSQL seed 验证空范围、跨项目资料、处理中资料和不可检索资料均展示固定错误文案，且不产生来源卡片或题目历史；真实 DashScope 新题成功检索、无可靠来源真实检索和历史重新检索 E2E 仍未验证。 | v0.2.0 实现推进 | `Makefile`、`frontend/e2e/v010.spec.ts`、`scripts/seed_question_scope_errors.py`、`items/feature-005-question-workflow.md` |
| 2026-06-02 | 落地题目历史与重新检索前端 E2E 切片：新增题目工具栏重新检索入口、固定检索 loading 状态和失败原因展示，新增 `make verify-e2e SCENARIO=v020-question-history-research`，使用真实 Web、FastAPI 和 PostgreSQL seed 验证历史题回看旧来源、重新检索调用真实后端并在缺少 embedding 凭据时清空旧来源、展示后端失败原因；固定向量成功替换仍由 DB/API 门禁覆盖，当时真实 DashScope 成功路径未验证。 | v0.2.0 实现推进 | `Makefile`、`frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`scripts/seed_question_history_research.py`、`items/feature-005-question-workflow.md` |
| 2026-06-02 | 落地视觉证据 manifest 与审美审计记录真实校验 target：新增 `scripts/verify_visual_gate.py` 并接入 `make verify-visual CHECK=visual-evidence-manifest` 和 `CHECK=aesthetic-audit-record`，缺少 manifest 或审计记录时按契约失败；截图矩阵生成、视觉硬错误检查和人工审美审计仍保持阻塞。 | v0.2.0 实现推进 | `Makefile`、`scripts/verify_visual_gate.py`、`items/gate-001-visual-quality.md` |
...
| 2026-06-02 | 落地 v0.2.0 证据包生成切片：`make evidence-package-with-tests` 生成固定 `tmp/v0.2.0-evidence-latest.md`，记录视觉证据摘要和每条必需命令的退出码、执行时间、数据准备命令、证据路径和结论；当时因 `v020-full-regression` 缺少 `DASHSCOPE_API_KEY` 失败，证据归档发布门禁保持阻塞。 | v0.2.0 实现推进 | `Makefile`、`scripts/collect_evidence.py`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 落地视觉系统聚合门禁切片：新增 `make verify-visual CHECK=visual-system`，串行执行截图矩阵、视觉硬错误、证据 manifest 和人工审美审计记录校验，作为 `design-001` 视觉系统验收的真实入口；核心闭环、设计 token 统一组件清单和总回归仍保持各自行级阻塞。 | v0.2.0 实现推进 | `Makefile`、`items/design-001-frontend-rebuild.md` |
| 2026-06-02 | 落地处理阶段 DB 与运行中视觉门禁切片：新增 `make verify-db CHECK=v020-processing-stages`，使用真实 `process_document`、PDF 文字层提取和 PostgreSQL/pgvector 验证 `embedding`、`indexing`、`completed` 阶段推进；新增 `make verify-visual CHECK=processing-progress-visual`，使用真实 Web/API/DB seed 生成处理轨道运行中截图并检查无硬错误；真实 DashScope/Redis worker 完成 E2E `v020-processing-progress` 仍保持阻塞。 | v0.2.0 实现推进 | `Makefile`、`scripts/verify_db.py`、`frontend/e2e/v010.spec.ts`、`items/feature-003-processing-progress.md` |
| 2026-06-02 | 完成 v0.1.0 项目名样例迁移验证切片：`make verify-db CHECK=v020-project-name-migration` 创建真实临时 PostgreSQL 数据库和 v0.1.0 形态 `projects` 表，执行当前迁移脚本，验证空名、NULL 名、trim、80 字符截断、重名后缀、workspace/updated_at 补齐、项目名约束、workspace/name 唯一索引和关键表可读；数据迁移与重名项目迁移矩阵行更新为通过。 | v0.2.0 实现推进 | `scripts/migrate.py`、`scripts/verify_db.py`、`items/feature-001-project-management.md`、`items/data-001-v020-model-api.md` |
| 2026-06-02 | 同步总体验收门禁自动化测试状态：根据已通过的 `make test`、`make verify-spec` 和 `make verify-secrets`，更新 `gate-002` 自动化测试矩阵实际结果和证据；因 `v020-full-regression`、真实 DashScope 成功路径和若干 `SCENARIO=v020-*` 仍未实现或未验证，该行继续保持阻塞。 | v0.2.0 spec 状态维护 | `items/gate-002-v020-validation.md` |
| 2026-06-02 | 落地核心闭环和总回归 E2E 入口切片：新增 `v020-core-loop` 与 `v020-full-regression` Playwright 场景，覆盖真实项目创建、固定 PDF 上传、Redis/RQ 处理、题目检索、来源结果、来源阅读、PDF iframe、专注模式、无可靠来源行动入口和来源页码导航；该命令必须走完整 `dev_check`，真实 DashScope 成功路径通过证据仍保持阻塞。 | v0.2.0 实现推进 | `Makefile`、`frontend/e2e/v010.spec.ts`、`items/design-001-frontend-rebuild.md`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 落地正常处理进度 E2E 入口切片：新增 `v020-processing-progress` Playwright 场景，使用真实 Web、FastAPI、Redis/RQ、PostgreSQL 和固定 PDF 上传，检查纸页入库组件、处理轨道六节点、最终完成状态、失败字段清空、可检索和 chunk 持久化；真实 DashScope 成功路径通过证据仍保持阻塞。 | v0.2.0 实现推进 | `frontend/e2e/v010.spec.ts`、`items/feature-003-processing-progress.md` |
| 2026-06-02 | 补齐资料详情重新处理入口显示条件：`completed`、`failed`、`unsupported` 三态资料详情展示重新处理入口，`processing` 状态不展示；`v020-document-detail-fields` 和 `v020-processing-polling-coalesced` 覆盖该显示契约，重处理 worker 完成路径仍保持阻塞。 | v0.2.0 实现推进 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`items/feature-002-document-management.md` |
| 2026-06-02 | 落地资料重处理 E2E 入口切片：新增 `v020-document-reprocess` Playwright 场景，使用真实 Web、FastAPI、Redis/RQ、PostgreSQL 和固定 PDF 上传，覆盖已完成资料点击重新处理、状态回到 uploaded、界面标记重新处理中、worker 再次完成、失败字段清空、可检索和 chunk 持久化；真实 DashScope 成功路径通过证据仍保持阻塞。 | v0.2.0 实现推进 | `frontend/app/page.tsx`、`frontend/e2e/v010.spec.ts`、`items/feature-002-document-management.md` |
| 2026-06-02 | 落地资料范围检索 E2E 入口切片：新增 `v020-document-scope-search` Playwright 场景，使用真实 Web、FastAPI、Redis/RQ、PostgreSQL 和两份固定 PDF 上传，覆盖指定资料多选、请求体 `document_ids` 只包含选中资料、响应来源只来自选中资料；真实 DashScope 成功路径通过证据仍保持阻塞。 | v0.2.0 实现推进 | `frontend/e2e/v010.spec.ts`、`items/feature-002-document-management.md`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 落地新题检索与无可靠来源 E2E 入口切片：新增 `v020-question-search` 和 `v020-question-no-source` Playwright 场景，使用真实 Web、FastAPI、Redis/RQ、PostgreSQL 和固定 PDF 上传，分别覆盖新题检索 loading/按钮禁用/来源置信层级，以及无可靠来源响应/行动入口/不展示伪来源；真实 DashScope 成功路径通过证据仍保持阻塞。 | v0.2.0 实现推进 | `frontend/e2e/v010.spec.ts`、`items/feature-005-question-workflow.md`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 同步数据/API 条目局部状态：根据已落地的资料重处理、题目检索、无可靠来源和历史重新检索真实 Playwright 入口，更新 `data-001` 中重处理与重新检索一致性、题目接口字段两条矩阵实际结果；真实 DashScope 成功路径通过证据仍保持阻塞。 | v0.2.0 spec 状态维护 | `items/data-001-v020-model-api.md`、`frontend/e2e/v010.spec.ts` |
| 2026-06-02 | 同步总回归与题目工作流阻塞证据：根据当时 `make evidence-package-with-tests` 生成的 `tmp/v0.2.0-evidence-latest.md`，明确总回归入口因缺少 `DASHSCOPE_API_KEY` 失败于 `dev_check`；同步 `feature-005` 题目接口契约行的前端入口状态，结论继续保持阻塞。 | v0.2.0 spec 状态维护 | `items/design-001-frontend-rebuild.md`、`items/feature-005-question-workflow.md`、`items/gate-002-v020-validation.md`、`tmp/v0.2.0-evidence-latest.md` |
| 2026-06-02 | 强化 v0.2.0 spec gate：`make verify-spec` 现在校验 README 已实现 target 清单与 DB/API/E2E/visual 源码支持 target 双向一致，并阻止真实 DashScope 成功路径场景进入 `--skip-embedding` 白名单；本地 `make test`、`make verify-spec` 和 `make verify-secrets` 已通过，整体发布仍因真实 DashScope 成功路径保持阻塞。 | v0.2.0 spec 状态维护 | `scripts/verify_release_gate.py`、`scripts/tests/test_gate_tools.py`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 补强 v0.2.0 spec gate 测试覆盖：新增 DashScope 成功路径场景误入 `--skip-embedding` 白名单的负向测试，并将 `make verify-spec` 成功输出同步为结构、target inventory 和 DashScope skip 白名单检查均通过；本地 `scripts/tests/test_gate_tools.py`、`make verify-spec` 和 `make verify-secrets` 已通过。 | v0.2.0 spec 状态维护 | `scripts/verify_release_gate.py`、`scripts/tests/test_gate_tools.py` |
| 2026-06-02 | 对齐无文字层处理文案：真实 `process_document` 路径改为使用 v0.2.0 固定 `no_text_layer` 失败原因 `PDF 无可提取文字层，v0.2.0 不进入 OCR`，并新增测试防止旧 `v0.1.0 不进入 OCR` 文案回归；该切片不影响真实 DashScope 成功路径阻塞结论。 | v0.2.0 实现推进 | `backend/app/processing.py`、`scripts/tests/test_gate_tools.py`、`items/feature-002-document-management.md`、`items/data-001-v020-model-api.md` |
| 2026-06-02 | 对齐 DashScope 缺凭据错误文案：`embed_texts`、`dev_check` 和 `doctor` 的缺少 `DASHSCOPE_API_KEY` 输出改为版本中性的 Suton DashScope embedding 文案，并新增测试防止 v0.2.0 证据输出继续绑定 v0.1.0 表述；当时真实 DashScope 成功路径保持阻塞。 | v0.2.0 实现推进 | `backend/app/embedding.py`、`scripts/dev_check.py`、`scripts/doctor.py`、`backend/tests/test_embedding.py`、`scripts/tests/test_gate_tools.py` |
| 2026-06-02 | 对齐本地运行诊断 CLI 描述：`scripts/doctor.py` 的 argparse 描述改为版本中性 `Suton local runtime diagnostics`，并新增 gate 测试防止 v0.2.0 本地诊断入口继续显示 v0.1.0 绑定标题；当时真实 DashScope 成功路径保持阻塞。 | v0.2.0 实现推进 | `scripts/doctor.py`、`scripts/tests/test_gate_tools.py` |
...
| 2026-06-02 | 修正 v0.2.0 证据包视觉摘要时序：`scripts/collect_evidence.py` 改为先执行证据命令并收集命令章节，再读取视觉 manifest 和人工审美审计摘要，避免 `tmp/v0.2.0-evidence-latest.md` 顶部 manifest commit 落后于本轮 `screenshot-matrix`；当时本地重新生成证据包因 `v020-full-regression` 缺少 `DASHSCOPE_API_KEY` 失败，证据归档继续阻塞。 | v0.2.0 实现推进 | `scripts/collect_evidence.py`、`scripts/tests/test_gate_tools.py`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 增加集中阻塞确认清单：README 明确剩余真实成功路径必须取得有效 DashScope `DASHSCOPE_API_KEY` 后重新执行，并列出受影响的处理进度、资料重处理、资料范围检索、题目检索、核心闭环、总回归和证据包命令；相关矩阵和发布门禁继续保持阻塞。 | v0.2.0 spec 状态维护 | `README.md`、`acceptance-checklist.md` |
| 2026-06-02 | 强化集中阻塞清单 spec gate：`make verify-spec` 新增校验，要求 README 集中阻塞确认清单包含全部真实 DashScope 成功路径命令、`make evidence-package-with-tests`、禁止 mock/固定向量/`--skip-embedding` 降级和 secret 禁止项；该检查只防止清单漂移，不代表真实 DashScope 成功路径已通过。 | v0.2.0 spec 状态维护 | `scripts/verify_release_gate.py`、`scripts/tests/test_gate_tools.py` |
| 2026-06-02 | 同步自动化门禁状态描述：`gate-002` 自动化测试矩阵和 README 总规则已对齐最新 `make verify-spec` 覆盖范围，明确 target inventory、DashScope skip 白名单、集中阻塞清单、有效 `DASHSCOPE_API_KEY`、no-mock/no-secret 约束均由 gate 工具测试覆盖；真实 DashScope 成功路径和整体发布仍保持阻塞。 | v0.2.0 spec 状态维护 | `README.md`、`items/gate-002-v020-validation.md` |
| 2026-06-02 | 收紧 target inventory 源码提取：`make verify-spec` 现在只从 `scripts/verify_db.py` 和 `scripts/verify_api_contract.py` 的 `main()` 分发字典读取 v0.2.0 DB/API target，测试覆盖 fixture 字典里的 `v020-*` key 不会被误认成已支持 target；该门禁仍只防止清单漂移，不代表真实 DashScope 成功路径已通过。 | v0.2.0 spec 状态维护 | `scripts/verify_release_gate.py`、`scripts/tests/test_gate_tools.py` |
