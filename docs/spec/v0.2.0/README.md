...
近期新增已实现 `make verify-e2e SCENARIO=v020-upload-indeterminate-progress`、`make verify-visual CHECK=upload-indeterminate-progress`、`make verify-e2e SCENARIO=v020-question-scope-errors`、`make verify-e2e SCENARIO=v020-question-history-research`、`make verify-visual CHECK=visual-evidence-manifest`、`make verify-visual CHECK=aesthetic-audit-record`、`make verify-visual CHECK=visual-hard-errors`、`make verify-visual CHECK=screenshot-matrix`、`make verify-visual CHECK=visual-system` 和强化后的 `make verify-visual CHECK=design-tokens`；其中前四项覆盖浏览器上传字节不可计算时的非百分比流动线、题目检索范围错误、历史题目回看和重新检索前端 E2E，`screenshot-matrix` 使用真实浏览器生成 16 张截图并覆盖 16 个固定状态和 5 个固定 viewport，`visual-hard-errors` 使用真实浏览器覆盖五个固定 viewport 的布局硬错误，manifest 和审美审计记录为视觉发布门禁的真实文件校验 target，`visual-system` 串行聚合截图矩阵、硬错误、manifest 和审美审计，`design-tokens` 现在同时校验固定 token、lucide 图标规则、禁用主 UI 组件库和统一组件清单定义/使用；其中截图矩阵、manifest、视觉硬错误、人工审美审计记录、视觉系统聚合门禁和设计 token/组件清单门禁已通过；其余未实现命令仍按上段约束保持阻塞。
