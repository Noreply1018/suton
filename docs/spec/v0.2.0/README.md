...
本轮新增已实现 `make verify-e2e SCENARIO=v020-upload-indeterminate-progress` 和 `make verify-visual CHECK=upload-indeterminate-progress`，用于覆盖浏览器上传字节不可计算时的非百分比流动线；其余未实现命令仍按上段约束保持阻塞。
