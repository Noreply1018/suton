...
| 文档与行为一致 | 本地仓库；记录 `git status --short --branch`、`git rev-parse HEAD`、`make env-info` 输出 | 所有功能实现完成 | 对照 `docs/spec/v0.1.0/README.md` 和 `items/` 检查实现行为；执行 `make test` | 实际行为不超出或偏离 spec，未完成项未标记完成 | 已验证：v0.1.0 功能条目均已标记完成，验证矩阵引用真实命令证据；自动化测试通过；UI 补充验证后 `make test` 为 `21 passed, 1 warning` | `docs/spec/v0.1.0/validation-2026-05-29.md`；`docs/spec/v0.1.0/validation-2026-05-31.md`；`make test` 通过 | 通过 |
