...
| 审计与提交门禁 | Git 仓库、subagent 工具、GitHub 远端；记录 `git status --short --branch` 和 `git ls-remote --heads origin main` | 所有验证通过 | 派发 subagent 严格审计；审计通过后执行 `git add`、`git commit`、`git push`；再执行 `git ls-remote --heads origin main` | 审计结论通过，Git 远端包含发布提交 | 本验证记录已完成本地审计和提交；最终远端复核必须在包含本行的提交推送后执行，不能由未提交文档自证 | 待包含本行的提交推送后执行 `git ls-remote --heads origin main` 复核 | 待验证 |
