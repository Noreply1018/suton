...
> 用户创建项目，上传带文字层的资料 PDF，系统解析并建立可检索资料索引；用户手动输入一道题，系统返回带来源的资料页码、片段和原页入口。
...
- v0.1.0 必须使用最小 embedding 检索；关键词检索只能作为兜底，不得替代 embedding 检索。
...
用户提供的 UI 草案可作为 v0.1.0 的视觉和信息架构参考，但不作为完整功能承诺。
...
- `make env-info` 必须输出 Node.js、包管理器、Python、数据库、pgvector、Redis 或队列实现、embedding provider、浏览器、操作系统和关键环境变量是否存在。
- `make reset-demo` 必须清理旧上传、旧索引和旧数据库记录。
- `make dev` 必须启动前端、后端、数据库连接和必要后台处理能力，或明确输出缺失条件并失败。
- `make migrate` 必须执行数据库迁移或初始化 schema。
- `make process-demo` 必须处理固定样例 PDF，并生成页面文本、chunk 和 embedding。
- `make verify-db` 必须执行数据库核验；实现可以通过 `CHECK=<name>` 指定检查项目。
- `make verify-e2e` 必须执行 v0.1.0 最小闭环浏览器或等价端到端验证。
...
- 必须使用固定样例 PDF 验证资料处理链路。样例 PDF 必须有文字层。
- 必须使用固定样例题目验证题目到资料依据的检索结果。
- 每条资料依据必须包含文件名、页码、原文片段、排序分数或排序依据、原页入口。
- 没有明确来源的结果不得进入前端列表。
